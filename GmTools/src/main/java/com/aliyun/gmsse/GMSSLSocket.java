...
public class GMSSLSocket extends SSLSocket {
    ...
    private static final String TAG = "GMSSLSocket";
    ...
    static {
        // setup suites
        supportedSuites.add(CipherSuite.GMTLS_SM2_WITH_SM4_SM3);

        // setup protocols
        supportedPtrotocols.add(ProtocolVersion.GMTLS_1_1);
    }
    ...
    private void initialize() {
        ...
        session.protocol = ProtocolVersion.GMTLS_1_1;
    }
    ...
    public void setEnabledProtocols(String[] protocols) {
        ...
        for (int i = 0; i < protocols.length; i++) {
            if (!(protocols[i].equalsIgnoreCase(ProtocolVersion.GMTLS_1_1_NAME))) {
        ...
        synchronized (session.enabledProtocols) {
            ...
            for (int i = 0; i < protocols.length; i++) {
                session.enabledProtocols.add(ProtocolVersion.GMTLS_1_1);
            }
        }
    }
    ...
    public void startHandshake() throws IOException {
        long handshakeStartTime = SystemClock.elapsedRealtime();
        Log.d(TAG, "startHandshake: 开始握手");
        ...
        long handshakeEndTime = SystemClock.elapsedRealtime();
        float handshakeDuration = (handshakeEndTime - handshakeStartTime) / 1000F;
        Log.d(TAG, String.format("startHandshake: 握手结束, 耗时: %.3f s", handshakeDuration));
    }
    ...
    private void sendFinished() throws IOException {
        ProtocolVersion version = ProtocolVersion.GMTLS_1_1;
    ...
    private void sendClientHello() throws IOException {
        ...
        byte[] randomBytes = session.random.generateSeed(28);
