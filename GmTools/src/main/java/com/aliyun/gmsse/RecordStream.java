...
public class RecordStream {
    ...
    public Record read(boolean encrpted) throws IOException {
        ...
        /*
          HTTP 1.1 长连接时，OkHttp 中的 ExchangeFinder.findHealthyConnection() --->  RealConnection.isHealthy() 会触发
            Socket 健康检测，这时就会等待服务端接口数据返回， 直到到达  OkHttp 设置的读取超时，  导致接口请求耗时异常。

          针对此问题的解决方案：
                1. 服务端升级到 HTTP 2.0 协议
                2. 客户端关闭长连接，每次请求都建立新的连接，  添加请求头  Connection: close  即可
        * */
        int type = input.read();
